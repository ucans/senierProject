...
from getLR import repeatLR

from accuracy import word_error_rate,sentence_acc
...
def run_epoch(
    ...
    with tqdm(
        ...
        for d in data_loader:
            ...
            output_2=output['dec_result']
            decoded_values = output_2.transpose(1,2)
...
def main(config_file):
    ...
    # if train last model, set model to load
    path ="./model.pth"
    is_cuda = torch.cuda.is_available()
    checkpoint = load_model(path, cuda=is_cuda)
    
    #checkpoint = (
    #    load_model(options.checkpoint, cuda=is_cuda)
    ...
    if model_checkpoint:
        ...
        # symbol accuracy graph
        epochAxis=np.arange(0, len(checkpoint["train_symbol_accuracy"]))
        plt.title("SYMBOL ACCURACY")
        plt.plot(epochAxis, checkpoint["train_symbol_accuracy"], epochAxis, checkpoint["validation_symbol_accuracy"], "r-")
        plt.legend(["TRAIN","VALIDATION"])
        plt.show()

        # symbol accuracy graph
        epochAxis=np.arange(0, len(checkpoint["train_sentence_accuracy"]))
        plt.title("SENTENCE ACCURACY")
        plt.plot(epochAxis, checkpoint["train_sentence_accuracy"], epochAxis, checkpoint["validation_sentence_accuracy"], "r-")
        plt.legend(["TRAIN","VALIDATION"])
        plt.show()


        # loss graph
        epochAxis=np.arange(0, len(checkpoint["train_losses"]))
        plt.title("LOSS")
        plt.plot(epochAxis, checkpoint["train_losses"], epochAxis, checkpoint["validation_losses"], "r-")
        plt.legend(["TRAIN","VALIDATION"])
        plt.show()
    ...
    writer = init_tensor(name=options.prefix.strip("-"))
